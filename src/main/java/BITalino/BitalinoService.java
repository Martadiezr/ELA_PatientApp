...
public class BitalinoService {
    ...
    public Signal acquireSignal(TypeSignal type, int clientId, int seconds) throws Throwable {
        ...
        BITalino bitalino = null; // BITalino object to manage the device connection
        ...
        try {
            ...
            // Check if a specific MAC Address/Port was provided in the constructor
            if (this.macAddress != null && !this.macAddress.trim().isEmpty()) {
                foundPort = this.macAddress; // Use the provided address directly
            } else {
                // Perform auto-search if no address was provided
                System.out.println("Searching BITalino...");
                SerialPort[] allPorts = SerialPort.getCommPorts();

                // First attempt: Look for 'BITalino' and 'cu.' (often used for macOS serial ports)
                ...
                // Second attempt: Look for just 'BITalino' in the port name
                if (foundPort == null) {
                    for (SerialPort p : allPorts) {
            ...
            // Check if a port was successfully found
            if (foundPort == null) {
